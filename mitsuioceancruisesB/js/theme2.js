...
$(".banner-img").slick({
  ...
  (function () {
    ...
    function adjustHeight(el) { wrapper.style.height = el.offsetHeight + "px"; }
    ...
    function switchTo(targetId, direction="left") {
      ...
      if (t) { t.classList.add("active"); scrollTabIntoView(t); }
      ...
      setTimeout(() => { current.classList.remove("exit-left", "exit-right"); }, 1000);
      activeId = targetId;
    }
    function autoPlay() { autoTimer = setInterval(() => {
      let idx = ids.indexOf(activeId);
      switchTo(ids[(idx + 1) % ids.length], "left");
    }, interval); }
    function restartAutoPlay() { clearInterval(autoTimer); autoPlay(); }
    ...
    tabs.forEach(btn => {
      btn.addEventListener("click", () => {
        ...
        const dir = ids.indexOf(target) > ids.indexOf(activeId) ? "left" : "right";
        switchTo(target, dir);
        restartAutoPlay();
      });
    });
    ...
    wrapper.addEventListener("touchend", e => {
      ...
      let idx = ids.indexOf(activeId);
      switchTo(diff < 0 ? ids[(idx + 1) % ids.length] : ids[(idx - 1 + ids.length) % ids.length],
               diff < 0 ? "left" : "right");
      restartAutoPlay();
    });
  })();
